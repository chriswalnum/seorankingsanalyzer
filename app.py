...
def generate_html_report(results, target_url, logo_base64=""):
    template_string = """
    <!DOCTYPE html>
    <html lang="en">
    <head>
        ...
        <style>
            ...
            .header { text-align: center; margin-bottom: 3rem; border-bottom: 2px solid #2563eb; padding-bottom: 1rem; }
            .header img { max-height: 100px; margin-bottom: 10px; }
            .header h1 { color: #2563eb; font-size: 28px; margin-bottom: 0.5rem; }
            .metrics { display: grid; grid-template-columns: repeat(3, 1fr); gap: 1.5rem; margin: 2rem 0; }
            .metric-card { background: #f8fafc; padding: 1.5rem; border-radius: 8px; border: 1px solid #e2e8f0; text-align: center; }
            .section-title { color: #2563eb; font-size: 24px; margin: 2.5rem 0 1.5rem; border-bottom: 2px solid #e2e8f0; padding-bottom: 0.5rem; }
            table { width: 100%; border-collapse: collapse; margin: 1.5rem 0; }
            th { background: #f1f5f9; color: #1e40af; font-weight: 600; text-align: left; }
            th, td { padding: 12px; border: 1px solid #e2e8f0; }
            .status-badge { display: inline-block; padding: 4px 8px; border-radius: 4px; font-size: 14px; font-weight: 500; }
    ...
    <body>
        ...
        <!-- (Report content such as metrics, tables, etc.) -->
        <footer style="text-align: center; margin-top: 2rem; font-size: 12px; color: #888;">
            by Chris Walnum
        </footer>
    </body>
    </html>
        ...
        if logo_base64:
            st.markdown(f"""
                <div style="display: flex; align-items: center; margin-bottom: 20px;">
                    <img src="data:image/png;base64,{logo_base64}" style="max-height: 80px; margin-right: 10px;">
                    <div style="color: #262730; font-size: 1.8em; font-weight: 600;">
                        SEO Rankings Analyzer Pro <span style="font-size: 0.8em; color: #888;">by Chris Walnum</span>
                    </div>
                </div>
            """, unsafe_allow_html=True)
        else:
            ...
            for result in results:
                with st.expander(f"{result['keyword']} in {result['location']}"):
                    for idx, loc in enumerate(result['local_results'], 1):
                        ...
                        st.markdown("---")
        html_report = generate_html_report(results, target_url, logo_base64)
